...
fn filter_play_groups(
    ...
    let min_play_count: u64 = 30;
    let min_ms_played: u64 = 60 * 1000 * min_play_count;

    hm.into_iter()
        .filter(|(s, pg)| {
            pg.get_agg_data().play_count >= min_play_count
                && pg.get_agg_data().ms_played.total >= min_ms_played
        })
        .collect()
}
...
fn main() -> Result<()> {
    ...
    let mut intersection_songs: Vec<Box<dyn PlayGroup>> = intersection_keys
        ...
        .filter_map(|(key, combined_song)| Some(combined_song))
        .collect();
